...
const Captcha: React.FC<ICaptchaProps> = (props) => {
  const { value = "", onChange, length = DEFAULT_LENGTH, autoFocus = false, theme = "line", className, style } = props;
  // 组件内部维护的输入框输入值
    ...
    <div className={`captcha captcha-theme-${theme} ${className || ""}`} style={style}>
      ...
      <div className="code-box" style={gutterStyle} onMouseDown={handleCodeBoxClick}>
        {codeArray.map((item, index, array) => {
          ...
          return (
            <div key={index} className={`item-content ${isItemActive ? "item-content-active" : ""}`} style={gutterStyle}>
              {item}
            </div>
          );
        })}
